...
c >>> Roughness scattering routines
      SUBROUTINE CALSRP(dlwran,nkr,pow)
...
c
      ...
      real    field(nnkr*4),dxfield(nnkr*4)
      ...
      COMPLEX k_r(np)
      COMPLEX dbdz(4,np),blc(4,np),cf1
      ...
      if (nkr.gt.nnkr) then
       write(6,*) '>>> NKR too large in CALSRP<<<'
       write(*,*) "nkr=",nkr,"max=",nnkr
       stop
      end if

      if (4.gt.nnkz) then
       write(6,*) '>>> nnkz too small in CALSRP <<<'
...
c
      if (nkmean.gt.nnkr) then
       write(6,*) '>>> NKMEAN too large in CALSRP<<<'
      ...
      do nnk=1,nkmean
        READ(45,END=7654) cf1,ln,(dbdz(j,nnk),j=1,4),(blc(j,nnk),j=1,4)
        k_r(nnk)=real(cf1)
      end do
 7654 write(6,'(1h ,a,i5,a,i5)') '>>> CALSRC: Read ',nkk,
...
c >>> Loop over the four bounday conditions
        do j=1,4
c         write(*,*) 'j=',j
         ...
         if (laytyp(ln).eq.2) then
          stop '>>> No roughness of Airy layer intefaces <<<'
         else
          do nnk=1,nkmean
C     build up vertical derivative of incident field boundary operator

           c_field(nnk) = dbdz(j,nnk)

C     build up rotation boundary operator of incident field

           c_dxfield(nnk) = blc(j,nnk) 
...
c >>> add negative spectrum
         ...
         if (nfmean) then
          do jj=2,nkr
           jn=nkr_i+2-jj
           c_field(jn)  =  sym * c_field(jj)
           c_dxfield(jn)= - sym * c_dxfield(jj)
          end do
         end if

C     transform incident field at depth to spatial domain
...
c     multiply roughness in spatial domain by incident field derivative

         do jj=1,nkr_i
            c_field(jj)=eta_x_z(jj)*
...
C     by rotation operatorfor  incident field

         do jj=1,nkr_i
            c_dxfield(jj)=eta_dx_z(jj)*
...
C     into deta_kx_z
            ...
            do jj=1,nkr_i
               dETA_KX_Z((jJ-1)*4+j)= c_dxfield(jj) 
            enddo


        end do
      
      write(6,*) '>>> CALSRP: Calling SOLSR <<<'

      call SOLSR(ln,dlwran,nkr,pow)

      RETURN        
                   
      END           

      SUBROUTINE CALSRC(dlwran,nkr,pow)
...
c
      ...
      if (nkr.gt.nnkr) then
       write(6,*) '>>> NKR too large in CALSRC<<<'
       write(*,*) "nkr=",nkr,"max=",nnkr
       stop
      end if

      if (4.gt.nnkz) then
       write(6,*) '>>> NNKZ too small in CALSRC <<<'
       write(*,*) "nkz=",4,"max=",nnkz
       stop
      end if
...
c
      ...
      do nnk=1,nkmean
        READ(45) cf1,ln,(dbdz(j,nnk),j=1,4),(blc(j,nnk),j=1,4)
        k_r(nnk)=real(cf1)
      end do
...
C >>> Loop over four boundary conditions
        do j=1,4
c         write(*,*) 'j=',j
         ...
         if (mbf_0.eq.0.or.(.not.bistat)) then
          do nnk=1,nkr_i
           c_field(nnk)=0.0
           c_dxfield(nnk)=0.0
          enddo
          if (laytyp(ln).eq.2) then
           stop '>>> No roughness of Airy layers <<<'
          else
           do nnk=1,nkmean
...
C     build up horizontal derivative of incident field
          ...
          if (freq.eq.freq1) then
           do jj=1,nkr_i
            write(68,*) real(c_field(jj)),aimag(c_field(jj))
            write(69,*) real(c_dxfield(jj)),aimag(c_dxfield(jj))
           end do
          end if

C     transform incident field derivative to spatial domain
          ...
          if (freq.eq.freq1) then
           do jj=1,nkr_i
            write(70,*) real(c_field(jj)),aimag(c_field(jj))
           end do
          end if
C     transform rotationoperator of inc field to spatial domain
          ...
          if (freq.eq.freq1) then
           do jj=1,nkr_i
            write(71,*) real(c_dxfield(jj)),aimag(c_dxfield(jj))
           end do
          end if
...
c            ex=cexpt(cmplx(0e0,-rk))
             ...
             if (j.eq.1.or.j.eq.3) then
c >>> even boundary operators (w,sigma_zz)
...
c >>> odd boundary operators (u,sigma_rz)
              c_field(jr)=c_field(jr) + c_fld_tmp(jk)*awg*
     &          (-ai*wn*bes_J1*dk_inc - sqrtk(jk)*ex*sqrtri(jr))   
              c_dxfield(jr)=c_dxfield(jr) + c_dxfld_tmp(jk)*awg*
     &          ( wn*bes_J0*dk_inc - sqrtk(jk)*ex*sqrtri(jr))   
            ...
            do jk=1,nkmean
             ...
             if (j.eq.1.or.j.eq.3) then
              ...
              if (jk.eq.1) then
               c_dxfield(jr)=0.5*wn*c_dxfld_tmp(jk)*dk_inc
               c_field(jr)=0e0
              else
               c_dxfield(jr)=c_dxfield(jr) + wn*dk_inc*c_fld_tmp(jk)
              end if
             end if
            end do
           end if
...
c        write(6,*) '      Interpolation'
     ...
     c           (jj-1)*dlr)
           ...
           if (jj.gt.1) then
            cfield(nkr_i+2-jj)=sym*field_int
           end if
          end do
     ...
     c                     (nkr)*dlr)
          ...
          cfield(nkr+1)=(1.0+sym)*field_int
...
c     grid of roughness realization. Antisymmetric
     ...
     c           (jj-1)*dlr)
            ...
            if (jj.gt.1) then
             cdxfield(nkr_i+2-jj)=-sym*dxfield_int
            end if
          end do

          field_int=cintpl(c_dxfield,0.,dk_inc*nkr_i*oneo2pi,nkr_i,
     c                     (nkr)*dlr)

          cdxfield(nkr+1)=(1.0-sym)*field_int
...
c >>> read interpolated incident field from file 47
...
c       write(6,*) '      Multiply by roughness'
            do jj=1,nkr_i
               c_field(jj)=eta_x_z(jj)*cfield(jj)
             if (freq.eq.freq1) then
              write(90,*) real(cfield(jj)),aimag(cfield(jj))
              write(92,*) ETA_x_Z(jj)
             end if
            enddo

            c_field(nkr+1)=eta_x_z(nkr+1)*cfield(nkr+1)
...
c       write(6,*) '      Multiply dx by roughness'
            ...
            do jj=1,nkr_i
               c_dxfield(jj)=eta_dx_z(jj)*cdxfield(jj)
             if (freq.eq.freq1) then
              write(91,*) real(cdxfield(jj)),aimag(cdxfield(jj))
              write(93,*) ETA_dx_Z(jj)
             end if
            enddo
            c_dxfield(nkr+1)=eta_dx_z(nkr+1)*cdxfield(nkr+1)
...
c              ex2=cexpt(cmplx(0E0,-rk))
             ...
             if (j.eq.1.or.j.eq.3) then
c >>> Symmetric boundary operators
...
c >>> Antisymmetric boundary operators
              c_field(jq)=c_field(jq)+c_fld_tmp(jr)*awg*
     &           (-ai*rangem*bes_J1*dlr-(sqrtr(jr)*ex1-sqrtr(jrm)*ex2)*
     &           sqrtq(jq))
              c_dxfield(jq)=c_dxfield(jq)+c_dxfld_tmp(jr)*awg*
     &           (-ai*rangem*bes_J1*dlr-(sqrtr(jr)*ex1-sqrtr(jrm)*ex2)*
     &           sqrtq(jq))
...
C     into deta_kx_z
             ...
             do jj=1,nkr_i
               ...
               deta_kx_z((jj-1)*4+j)= c_dxfield(jj) 
c               deta_kx_z((jj-1)*4+j)= 0e0 
              if (freq.eq.freq1) then
               write(85,*) real(c_dxfield(jj)),aimag(c_dxfield(jj))
              end if
             enddo
      ...
      SUBROUTINE SOLSR(ln,dlwran,nkr,pow)
...
C *** FILL IN SCATTERING RIGHT HAND SIDE
       ...
       CC4=CNUL
       q_z = -ai*alfa(ln)
c >>> find relevant wavenumber bin
       iq=(wvno+dlwvno*0.1)/dlwvno + 1
       if (iq.lt.1) then
        iq=2*nkr+iq
       else
        iq=iq
       ...
       do j=1,2
         r(ln-1,j,1) = r(ln-1,j,1) +
     &                (peta_kx_z((iq-1)*4+j)+deta_kx_z((iq-1)*4+j))*
     &                 pow*disnrm
       end do
     ...
     &                 pow*strnrm*pcorr
       ...
       write(95,*) real(ss(ln-1,3,1)),aimag(ss(ln-1,3,1))
...
c     insert transformed product into peta_kx_z
            ...
            do jj=1,nkr_i
               ...
               pETA_KX_Z((jJ-1)*NKZ+j)=c_field(jj)
               if (freq.eq.freq1) then
C                write(84,*) real(c_field(jj)),aimag(c_field(jj))
               end if
            enddo
...
C     write(*,*) 'STRNRM=',strnrm,'DISNRM=',disnrm
            ...
            R(ln-1,1,1)=R(ln-1,1,1)
     &           - DISNRM*ACO(LN)*CC1*BIRYDU(ln)
            R(ln-1,3,1)=R(ln-1,3,1)
     &           - STRNRM*CON1(LN)*CC1*BIRYU(ln)
            r(ln-1,6,1) = - r(ln-1,3,1)
            
C     *** Lower interface
            
            R(ln,1,1)=R(ln,1,1)
     &           + DISNRM*ACO(LN)*CC2*AIRYDL(ln)
            R(ln,3,1)=R(ln,3,1)
     &           + STRNRM*CON1(LN)*CC2*AIRYL(ln)
            r(ln,6,1) = - r(ln,3,1)
            
         else
            
            R(ln-1,1,1)=R(ln-1,1,1)
     &           - DISNRM*ACO(LN)*CC1*AIRYDU(ln)
            R(ln-1,3,1)=R(ln-1,3,1)
     &           - STRNRM*CON1(ln)*CC1*AIRYU(ln)
            r(ln-1,6,1) = - r(ln-1,3,1)
            
C     *** Lower interface

            R(ln,1,1)=R(ln,1,1)
     &           + DISNRM*ACO(LN)*CC2*BIRYDL(ln)
            R(ln,3,1)=R(ln,3,1)
     &           + STRNRM*CON1(ln)*CC2*BIRYL(ln)
            r(ln,6,1) = - r(ln,3,1)
...
C *** 
         ...
         if (laytyp(ln).eq.1) then
          pot(1,int,1) =  pot(1,int,1) + cc2
          pot(3,int,1) =  pot(3,int,1) + cc1
         else
          if (real(aco(ln)).lt.0) then
           pot(1,int,1) =  pot(1,int,1) + cc2*AIRY(INT)
           pot(3,int,1) =  pot(3,int,1) + cc1*BIRY(INT)
           pot(2,int,1) =  pot(2,int,1) + cc2*AIRYD(INT)
           pot(4,int,1) =  pot(4,int,1) + cc1*BIRYD(INT)
          else
           pot(1,int,1) =  pot(1,int,1) + cc2*BIRY(INT)
           pot(3,int,1) =  pot(3,int,1) + cc1*AIRY(INT)
           pot(2,int,1) =  pot(2,int,1) + cc2*BIRYD(INT)
           pot(4,int,1) =  pot(4,int,1) + cc1*AIRYD(INT)
          endif
         endif
        end do
       end if
...
C     MODEL  3/2<MODEL<5/2 3-d spectral exponent for Goff-Jordan (2-D TO 3-D),
C            NEGATIVE FOR GAUSSIAN
...
C     COMPUTE SPATIAL SAMPLING INTERVALS
      ...
      if (depth.ne.0e0) then 
       DKZ=2.0*PI/DEPTH
      else
       dkz=1.0
      end if

      write(6,*) '>>> PV: Scattering layer <<<'
      write(6,*) '>>> Patch diameter: ', range,' m'
      write(6,*) '>>> Range Samples:  ', nkr,' '
      write(6,*) '>>> Delta r:        ', dr,' m'
      write(6,*) '>>> Patch depth:    ', depth,' m'
      write(6,*) '>>> Depth Samples:  ', nkz,' '
...
c      IDUM2=-9
      ...
      DO 100 J=1,NKR_I
         ...
         DO 90 K=1,NKZ
            ...
            IF (K.LE.NKZ/2.or.nkz.eq.1) THEN
...
c >>> 3-D spectral model (Turgut, JASA 102(2), 833-852, 1997, eq.(7)

               frc=exp(gammln(model))/
...
c >>> 1-D spectral model (Turgut, JASA 102(2), 833-852, 1997, eq.(10)
     ...
     C              exp(gammln(model-1.5))
                ...
                PK=SQRT(LR/sqrt(PI)*frc*
     &             (1.0+(KR*KR*LRR))**(-model+1.0))
 ...
 100  CONTINUE
      ...
      if (nkz.eq.1) then
       call vsmul(temp_kx_kz,1,1e0/ppow,temp_kx_kz,1,2*nkz*nkr_i)
      end if
...
c     Symmetric roughness in cylindrical geometry
            ...
            do jj=2,nkr
               jn=nkr_i+2-jj
               c_field(jj)=eta_x_z((jJ-1)*NKZ+j)*cmplx(1.0,0.0)
            enddo
...
c     write(6,*) 'four1 to wn'
            ...
            call four1(c_field,nkr_i,1)
c     
c     >>> Integration constants consistent with DF transform (1/N)
            
            call vsmul(c_field,1,1.0/nkr_i,c_field,1,2*nkr_i)
...
C                      c_field(jr)=c_field(jr) - c_fld_tmp(jk)*rfk*
C     &           (cfk(jk)*ex1+cfk(jkn)*ex2)*sqrtr(jr)
...
C     write out residue of fourier subtration kdl 050900

C                     write(80,*) jr,real(c_field(jr)),aimag(c_field(jr))

C     write out normal result kdl 050900

C                    write(90,*) jr,real(c_field(jr)),aimag(c_field(jr))
            ...
            do jj=1,nkr
               jn=nkr_i+2-jj
               eta_x_z((jJ-1)*NKZ+j)=field((jj-1)*2+1)*ffc
               eta_dx_z((jJ-1)*NKZ+j)=dxfield((jj-1)*2+1)*ffc
               if (jj.gt.1) then
...
c               write(6,*) eta_x_z(j+nkz),eta_dx_z(j+nkz)
            eta_x_z(nkr*nkz+j)=field(nkr*2+1)*ffc
...
C            write(80,*)  eta_x_z((jJ-1)*NKZ+j)

C            write(81,*)  eta_dx_z((jJ-1)*NKZ+j)
