...
C        *****  CALCULATE REPLICA FIELDS 
         ...
         DO 703 ISRCZ=1,NSRCZ
          if (nsrcz.gt.1) then
           v(jlayz,jparz)=zsmin+(isrcz-1)*((zsmax-zsmin)/(nsrcz-1))
           if (contz) then
            v(jlayz-1,3)=-v(jlayz,2)
           end if
          end if
c          delz=v(jlayz,jparz)*1E-3
          DO 703 ISRCx=1,NSRCx
           if (nsrcx.gt.1) then
            v(jlayx,jparx)=xsmin+(isrcx-1)*((xsmax-xsmin)/(nsrcx-1))
            if (contx) then
             v(jlayx-1,3)=-v(jlayx,2)
            end if
           end if
c           delx=v(jlayx,jparx)*1E-3
            OFFDB=OFFDBS
            do i=1,numl
             laytyp(i)=laytsav(i)
            end do    
...
c >>> Make plot entry for Cramer-Rao bounds
     ...
     &             YLO,YUP,YINC,YDIV,ZTXT,'LIN',nsrcz*nsrcx)
       write(26,*) '>>> INHERENT BOUNDS <<<'
       do isrcz=1,nsrcz
        if (nsrcz.gt.1) then
         v(jlayz,jparz)=zsmin+(isrcz-1)*((zsmax-zsmin)/(nsrcz-1))
         if (contz) v(jlayz-1,3)=-v(jlayz,2)
        ...
        do isrcx=1,nsrcx
         ...
         call crbound(v(jlayx,jparx),v(jlayz,jparz),
...
c >>> Coherent Fisher info matrix
     ...
     &            real(cli(i,isrcx,isrcz))*real(cli(j,isrcx,isrcz)))
         ...
         call crbound(v(jlayx,jparx),v(jlayz,jparz),afish_f(1,1))
        end do
       end do
      end if
