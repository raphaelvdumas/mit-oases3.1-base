...
c >>> check for layer sequence consistency
       if (m.le.2) then
        v(1,1)=v(2,1)
       else if (v(m,1).lt.v(m-1,1)) then
        write(6,*) '>>> Layer sequence error, layer:', m
        stop
       end if
c >>> check for flow
       if (abs(v(m,5)+888.888).lt. 1e-3) then
        flow_flag(m)=.true.
        flow_vel(m)=v(m,5)
        v(m,5)=0
       else
...
c >>> Volume scattering
       if (clen(m).lt.-1e-10) then
        nvol=6
        backspace(1)
        READ(1,*) (V(M,N),N=1,6),(fac(j+(m-1)*nvol),j=1,nvol) 
        WRITE(6,351) (V(M,N),N=1,6)
        write(6,*) '>>> Volume scattering layer <<<'
        write(6,'(a,f6.1,a)') '>>> L_z  = ',-fac(1+(m-1)*nvol),' m'
        write(6,'(a,f6.1,a)') '>>> L_x  = ',-fac(2+(m-1)*nvol),' m'
        write(6,'(a,f6.1,a)') '>>> Skew = ', fac(3+(m-1)*nvol),' deg'
        write(6,'(a,f6.1,a)') '>>> Exp  = ', fac(4+(m-1)*nvol),'  '
        write(6,'(a,f6.1,a)') '>>> dc/c = ', fac(5+(m-1)*nvol),'  '
        write(6,'(a,f6.1,a)') '>>> Gamm = ', fac(6+(m-1)*nvol),'  '
        amod(m)=fac(4+(m-1)*nvol)
       else
        nvol=3
        backspace(1)
        READ(1,*) (V(M,N),N=1,6),(fac(j+(m-1)*nvol),j=1,nvol) 
        ...
        write(6,*) '>>> Rms = ',abs(rough(m))
        WRITE(6,*) '>>> L_x = ',CLEN(M)
        write(6,*) '>>> Exp = ',fac(3+(m-1)*nvol)
        amod(m)=fac(3+(m-1)*nvol)
       end if
      else
       clen(m)=0e0
       if (goff) then
      ...
      IF (CLEN(M).EQ.0.0) THEN
C     NON-KIRCHHOFF SCATTERING
         CLEN(M)=1E6
      END IF
      if (flow_flag(m)) then
       read(1,*) flow_vel(m)
       write(6,*) '>>> Flow speed:',flow_vel(m),' m/s'
      end if
...
c >>>  Check for auto-continuous SVP
       if (M.GT.2) then
         if (abs(V(M-1,3)+999.999).lt.1E-3) then
          if (abs(v(m-1,2)-v(m,2)).gt.1E-3) then
           V(M-1,3)=-V(M,2)
          else
...
c >>> check if iso velocity
       if (abs(v(m,2)+v(m,3)).lt.1e-2) then
        v(m,3)=0e0
        LAYTYP(M)=1
        NUMT(1)=NUMT(1)+1
        LAYT(NUMT(1),1)=M
...
C
     ...
     &  '>>>>>WARNING: UNPHYSICAL ATTENUATION, (As/Ap)*(Cs/Cp)**2>0.75'
        END IF
      else if (nint(v(m,2)).eq.-3.and.v(m,3).ge.0E0) then
c >>>
c >>> Cp =-3 ; Cs > 0
c >>> dispersive elastic medium
c >>>
       if (v(m,3).gt.0E0) then
        LAYTYP(M)=3
        disper(m)=.true.
        NUMT(3)=NUMT(3)+1
        LAYT(NUMT(3),3)=M
...
Cms  i.e., V(M,1), is used in subsequent calculations and must
Cms  therefore be entered correctly.
Cms
c >>>
c >>> Cp < 0 ; Cs < 0
c >>> 
      ELSE IF (V(M,2).LT.0E0.AND.V(M,3).LT.0E0) THEN
        LAYTYP(M)=5
        NUMT(5)=NUMT(5)+1
        LAYT(NUMT(5),5)=M
        NBL=NBL+1
...
Cms
c        V(M,6) = BPROP(1,M)
c        V(M,2) = SQRT(BPROP(10,M)*1.E-3/BPROP(1,M))
c        V(M,3) = SQRT(BPROP(9,M)*1.E-3/BPROP(1,M))
...
Cmss
      Call blprp(m)
      write(6,598) bfast(m), bshear(m), rho(m)
...
Cms
Cms  transversely isotropic layer - modified to require V(M,3) >= 0
Cms
c >>>
c >>> Cp < 0 ; Cs >= 0
c >>> 
      ELSE IF (V(M,2).LT.0E0.AND.V(M,3).GE.0E0) THEN
