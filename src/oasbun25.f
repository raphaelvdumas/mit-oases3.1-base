...
c ********************************************************
c
c      IMPLICIT COMPLEX*16 (A,B,Z)
      REAL*8 ZPR(2)
      EQUIVALENCE (ZPR(1),ZPP)
      COMPLEX ZS,AIS,BIS,AIPS,BIPS,ZETAS
      complex*16 Z,ZPP,AI,BI,AIP,BIP,ZETA
...
C      SOME PRECISION LOSS MUST OCCUR, BE FOREWARNED)
      ...
      SUBROUTINE KSCAIRY (ZS,AIS,BIS,AIPS,BIPS,ZETAS)
c ********************************************************
c *                       OASES                          *
c *  Ocean Acoustic and Seismic Exploration Synthetics   *
c *                   Copyright (C)                      *
...
c ********************************************************
c
c      IMPLICIT COMPLEX (A,B,Z)
      REAL ZPR(2)
      EQUIVALENCE (ZPR(1),ZPP)
      COMPLEX ZS,AIS,BIS,AIPS,BIPS,ZETAS
      complex Z,ZPP,AI,BI,AIP,BIP,ZETA
      COMPLEX PHSE13,PHSE23,PHSE16,PHSE56
      COMPLEX PHSC13,PHSC23,PHSC16,PHSC56
      DATA     PHSE13/( 0.5, 0.8660254037844387)/,
     1         PHSE23/(-0.5, 0.8660254037844387)/,
     2         PHSE16/( 0.8660254037844387, 0.5)/,
     3         PHSE56/(-0.8660254037844387, 0.5)/,
     4         PHSC13/( 0.5,-0.8660254037844387)/,
     5         PHSC23/(-0.5,-0.8660254037844387)/,
     6         PHSC16/( 0.8660254037844387,-0.5)/,
     7         PHSC56/(-0.8660254037844387,-0.5)/
      IRET=1
      Z=ZS
      ZPP=Z
      CALL KCLAIRY(Z,IRET,AI,BI,AIP,BIP,ZETA)
      IF (ZPR(2).GE.0.)  THEN
        BI  = 2.*PHSC16  * BI
        BIP = 2.*PHSC56  * BIP
      ELSE
      ...
      RETURN
      END
      SUBROUTINE KCLAIRY (Z,IRET, AI,BI,AIP,BIP,ZETA)
c ********************************************************
c *                       OASES                          *
c *  Ocean Acoustic and Seismic Exploration Synthetics   *
c *                   Copyright (C)                      *
...
c ********************************************************
c
      IMPLICIT COMPLEX (A,B,E,X,Z)
      PARAMETER (ONEOOF=1/1.5)
...
C      PARAMETER (TPIO3=2.094395102393195, I=(0.0,1.0))
      REAL TPIO3
      REAL ZPR(2)
      EQUIVALENCE (ZPR(1),ZPP)
      COMPLEX PHSE13,PHSE23,PHSE16,PHSE56
...
C   *********************************************************
C   *               *
C   *   SERIES SOLUTIONS FOR AI & BI WITHIN 2.5 OF ORIGIN   *
C   *               *
C   *********************************************************

      IF (ABS (Z) .LE. 2.5)  THEN
C          write(*,*) 'CALL KABSERIES (Z, AI,AIP,BI,BIP)'
          CALL KABSERIES (Z, AI,AIP,BI,BIP)
          ZETA = (0.0, 0.0)
          CALL KFORMAIRY (AI,BI,AIP,BIP,Z,ZETA,0,IRET)
...
C   ******************************************************************
      ...
      ELSE IF (ATAN2(ZPR(2),ZPR(1)) .LE. TPIO3)  THEN
          IF (ABS(ZP).GT.5.4211)  THEN          
C              write(*,*) 'CALL KABGAUSS (ZP, AI, AIP, BI, BIP, ZETA)'
              CALL KABGAUSS (ZP, AI, AIP, BI, BIP, ZETA)
              IF (ICONJG.EQ.1)  THEN              
                  ZETA = CONJG (ZETA)
                  AI  = CONJG (AI)
                  BI  = CONJG (BI)
              ...
              END IF
          ELSE
              IF ((0.0935*(REAL(ZP)-(-0.65))**2.0
     c            +0.0135*(REAL(ZP)-(-0.65))*(AIMAG(ZP)-.92)
     c            +0.0535*(AIMAG(ZP)-.92)**2.0).LT.1.0)  THEN
C              IF (ABS (ZP-(-0.65,0.92)) .LT. 4.5)  THEN
C                  write(*,*) 'CALL KASERIES (ZP, AI,AIP, ZETA)'
                  CALL KASERIES (ZP, AI,AIP, ZETA)
              ELSE
C                  write(*,*) 'CALL KAGAUSS (ZP, AI, AIP, ZETA)'
                  CALL KAGAUSS (ZP, AI, AIP, ZETA)
              END IF
              ZP2 = CONJG (ZP) * PHSE23
              IF ((0.0935*(REAL(ZP2)-(-0.65))**2.0
     c            +0.0135*(REAL(ZP2)-(-0.65))*(AIMAG(ZP2)-.92)
     c            +0.0535*(AIMAG(ZP2)-.92)**2.0).LT.1.0)  THEN
C              IF (ABS (ZP2-(-0.9,2.8)) .LT. 4.97)  THEN
C                  write(*,*) 'CALL KASERIES (ZP2, BI, BIP, ZETA2)'
                  CALL KASERIES (ZP2, BI, BIP, ZETA2)
              ELSE
C                  write(*,*) 'CALL KAGAUSS (ZP2, BI, BIP, ZETA2)'
                  CALL KAGAUSS (ZP2, BI, BIP, ZETA2)
              END IF
              IF (ICONJG.EQ.1)  THEN              
                  ZETA = CONJG (ZETA)
                  AI  = CONJG (AI)
              ...
              END IF
          END IF
          CALL KFORMAIRY (AI,BI,AIP,BIP,Z,ZETA,1,IRET)
...
C   **********************************************************
      ...
      ELSE 
          ...
          IF ((0.0935*(REAL(Z1)-(-0.65))**2.0
     c         +0.0135*(REAL(Z1)-(-0.65))*(AIMAG(Z1)-.92)
     c         +0.0535*(AIMAG(Z1)-.92)**2.0).LT.1.0)  THEN
C          IF (ABS (Z1-(-0.9,2.8)) .LT. 4.97)  THEN
C              write(*,*) 'CALL KASERIES (Z1, AI1, AIP1, ZETA1)'
              CALL KASERIES (Z1, AI1, AIP1, ZETA1)
          ELSE
C              write(*,*) 'CALL KAGAUSS (Z1, AI1, AIP1, ZETA1)'
              CALL KAGAUSS (Z1, AI1, AIP1, ZETA1)
          END IF
          IF ((0.0935*(REAL(Z2)-(-0.65))**2.0
     c         +0.0135*(REAL(Z2)-(-0.65))*(AIMAG(Z2)-.92)
     c         +0.0535*(AIMAG(Z2)-.92)**2.0).LT.1.0)  THEN
C          IF (ABS (Z2-(-0.9,2.8)) .LT. 4.97)  THEN
C              write(*,*) 'CALL KASERIES (Z2, AI2, AIP2, ZETA2)'
              CALL KASERIES (Z2, AI2, AIP2, ZETA2)
          ELSE
C              write(*,*) 'CALL KAGAUSS (Z2, AI2, AIP2, ZETA2)'
              CALL KAGAUSS (Z2, AI2, AIP2, ZETA2)
          END IF
          AI2  = CONJG (AI2)  
          AIP2 = CONJG (AIP2)
          ZETA2 = CONJG (ZETA2)
      ...
      RETURN
      END
      SUBROUTINE KABSERIES (Z, AI,AIP,BI,BIP)
c ********************************************************
c *                       OASES                          *
c *  Ocean Acoustic and Seismic Exploration Synthetics   *
c *                   Copyright (C)                      *
...
c
      ...
      REAL C1,C2,SQR3,FAC1,FAC2,FAC3
      DATA C1 /0.355028053887817/, C2 /0.258819403792807/,
     1          SQR3 /1.732050807568877/
...
C         write(*,*) 'ABSERIES, K=',K
      ...
      AI = AI + (C1*FC - C2*GC)
      BI = BI + (C1*FC + C2*GC)
      IF (ABS(FC).LT.1.E-10*ABS(AI) .AND. 
     &    ABS(GC).LT.1.E-10*ABS(AI))  GO TO 12
   10 CONTINUE
      WRITE (6,*) ' SERIES APPROXIMATION FAILED TO CONVERGE'
   ...
   12 BI = SQR3 * BI
      ...
      RETURN
      END
      SUBROUTINE KABGAUSS (Z, AI,AIP,BI,BIP,ZETA)
c ********************************************************
c *                       OASES                          *
c *  Ocean Acoustic and Seismic Exploration Synthetics   *
c *                   Copyright (C)                      *
...
c
      ...
      PARAMETER (ONEOPI=1.0/PI) 
      COMPLEX SQRTZ,SQRPIZ,CFAC,CFAC2,ONEOZ
      REAL ZPR(2)
      EQUIVALENCE (ZPR(1),ZPP)
      COMPLEX EYE
      REAL X2(2), W2(2), X4(4), W4(4)
      COMPLEX PHSE13,PHSE23,PHSE16,PHSE56
      COMPLEX PHSC13,PHSC23,PHSC16,PHSC56
      DATA     PHSE13/( 0.5, 0.8660254037844387)/,
     1         PHSE23/(-0.5, 0.8660254037844387)/,
     ...
     7         PHSC56/(-0.8660254037844387,-0.5)/
      ...
      DATA X2/1.0592469382112378,  3.6800601866153044E-2/,
     1     W2/3.1927194042263958E-2,  9.6807280595773604E-1/,
     2     X4/3.9198329554455091,  1.6915619004823504,
     3     5.0275532467263918E-1, 1.9247060562015692E-2/,
     4     W4/4.7763903057577263E-5, 4.9914306432910959E-3,
     5     8.6169846993840312E-2, 9.0879095845981102E-1/
     ...
     c        W2(2)*ZETA*C2
         BI=W2(1)*ZETA*C_1+
     c        W2(2)*ZETA*C_2
         AIP=W2(1)*X2(1)*SQRTZ*CC1+
     c        W2(2)*X2(2)*SQRTZ*CC2
         BIP=W2(1)*X2(1)*SQRTZ*CC_1+
     c        W2(2)*X2(2)*SQRTZ*CC_2
      ELSE
         C1=1.0/(ZETA+X4(1))
         C2=1.0/(ZETA+X4(2))
         C3=1.0/(ZETA+X4(3))
     ...
     c        W4(3)*ZETA*C_3+
     c        W4(4)*ZETA*C_4
         AIP=W4(1)*X4(1)*SQRTZ*CC1+
     c        W4(2)*X4(2)*SQRTZ*CC2+
     c        W4(3)*X4(3)*SQRTZ*CC3+
     c        W4(4)*X4(4)*SQRTZ*CC4
         BIP=W4(1)*X4(1)*SQRTZ*CC_1+
     c        W4(2)*X4(2)*SQRTZ*CC_2+
     c        W4(3)*X4(3)*SQRTZ*CC_3+
     c        W4(4)*X4(4)*SQRTZ*CC_4
      END IF
      CFAC=1.0/SQRPIZ
      ONEOZ=1.0/Z
      AIP = - AI *0.125 * CFAC * ONEOZ
     1      - AI * SQRPIZ * 0.5 * ONEOPI
     1      + AIP * 0.5 * CFAC
      BIP = - BI * 0.25 * CFAC * ONEOZ
     1      + BI * SQRPIZ * ONEOPI
     1      - BIP * CFAC
      AI = AI * 0.5 * CFAC
      BI = BI * CFAC
      ...
      RETURN
      END
      SUBROUTINE KASERIES (Z, AI, AIP, ZETA)
c ********************************************************
c *                       OASES                          *
c *  Ocean Acoustic and Seismic Exploration Synthetics   *
c *                   Copyright (C)                      *
...
C         write(*,*) 'ASERIES, K=',K
   ...
   12 ZETA = Z * SQRT(Z) * ONEOOF
      ...
      RETURN
      END
      SUBROUTINE KAGAUSS (Z, AI,AIP,ZETA)
c ********************************************************
c *                       OASES                          *
c *  Ocean Acoustic and Seismic Exploration Synthetics   *
c *                   Copyright (C)                      *
...
c
     ...
     1      + AIP * 0.5 * CFAC
      ...
      RETURN
      END
      SUBROUTINE KFORMAIRY (AI, BI, AIP, BIP, Z, ZETA, IRETIN, IRETOUT)
c ********************************************************
c *                       OASES                          *
c *  Ocean Acoustic and Seismic Exploration Synthetics   *
c *                   Copyright (C)                      *
...
C      SOME PRECISION LOSS MUST OCCUR, BE FOREWARNED)
      ...
      IF (IRETIN.LT.0)  THEN                  
         IF (ZPR(2).GE.0.)  THEN         
            IF (RZETA.GE.0.)  THEN             
               EXPTZETA = EXP (-2.*ZETA)
               BI  = PHSE16*(BI  - COMPI*AI*EXPTZETA)*0.5
               BIP = PHSE56*(BIP - COMPI*AIP*EXPTZETA)*0.5
            ELSE                  
               EXPTZETA = EXP (2.*ZETA)
               BI  = PHSE16*(BI*EXPTZETA  - COMPI*AI)*0.5
               BIP = PHSE56*(BIP*EXPTZETA - COMPI*AIP)*0.5
            END IF
         ELSE
            IF (RZETA.GE.0.)  THEN             
               EXPTZETA = EXP (-2.*ZETA)
               BI  = PHSC16*(BI  + COMPI*AI*EXPTZETA)*0.5
               BIP = PHSC56*(BIP + COMPI*AIP*EXPTZETA)*0.5
            ELSE                  
               EXPTZETA = EXP (2.*ZETA)
               BI  = PHSC16*(BI*EXPTZETA  + COMPI*AI)*0.5
               BIP = PHSC56*(BIP*EXPTZETA + COMPI*AIP)*0.5
            END IF
         END IF
      ...
      ELSE IF (IRETIN.EQ.0)  THEN             
         ...
         EXPMZETA = EXP (-ZETA)
         IF (ZPR(2).GE.0.)  THEN
            BI  = PHSE16*(BI  - COMPI*AI)*EXPMZETA*0.5
            BIP = PHSE56*(BIP - COMPI*AIP)*EXPMZETA*0.5
         ELSE
            BI  = PHSC16*(BI  + COMPI*AI)*EXPMZETA*0.5
            BIP = PHSC56*(BIP + COMPI*AIP)*EXPMZETA*0.5
         END IF
         AI  = AI*EXPZETA
         AIP = AIP*EXPZETA
      END IF
