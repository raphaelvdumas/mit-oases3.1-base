...
c     from roughness and volume inhomogeneities
      ...
      parameter (nkrexp=13,nkzexp=9,nkrm=2**nkrexp,nkzm=2**nkzexp,
     &           nkrz=2*nkrm*nkzm)
      INCLUDE 'compar.f'
      INCLUDE 'comnla.f'
      INCLUDE 'comnp.f'
...
C >>> interface info from file 45
     ...
     c     ,clen(intfce)
      ...
      if (volume) then
       write(*,*) 'nwvnor=',nwvnor
       write(*,*) 'nkrm=',nkrm
       if (nwvnor.gt.nkrm) stop '>>> Too many wavenumbers <<<'
       ln=intfce
       nvol=6
...
c
       do II=1,2*NWVNOR
        FCCS=-SQRT(RG2*P((II-1)*DLWH,INTFCE))
        if (ii.gt.1) then
         CRAN(II,1)=FCCS*EXP(AI*GASDEV(ISEED)*2*PI)
c         CRAN(II,1)=FCCS*EXP(AI*RAN2(ISEED)*2*PI)
        else
         CRAN(II,1)=FCCS
        end if
       end do
c >>> convert to spatial domain
       call cvmov(cran(1,1),2,cff(1,1),2,2*nwvnor)
       call cvconj(cran(2,1),2,cff(4*nwvnor,1),-2,2*nwvnor-1)
       cff(2*nwvnor+1,1) = cff(2*nwvnor,1)
       call cfft(cff,4*nwvnor,1)
c >>> taper upper 20%
       nwt=2*nwvnor*0.2
       nw1=2*nwvnor-nwt
       nw2=2*nwvnor+nwt+1
       anw=1e0/(nw2-nw1)
       do ii=nw1,nw2
        cff(ii,1)=cff(ii,1)*0.5*(1.0+cos(2*pi*(ii-nw1)*anw))
       end do
c >>> back to wavenumber
       call cfft(cff,4*nwvnor,-1)
       call cvmov(cff,2,cran,2,2*nwvnor)
       call vsmul(cran,1,1E0/(4e0*nwvnor),cran,1,4*nwvnor)
      else
c
c HS 980129. Cylindrical compensation. Only valid for monostatic
c
       write(6,*) 'Warning: Cylindrical compensation only valid '
       write(6,*) '         for monostatic receivers'
       do II=1,2*nwvnor
        FCCS=-SQRT(RG2*P((II-1)*DLWH,INTFCE))
        if (ii.gt.1) then
         CRAN(II,1)=FCCS*EXP(AI*gasdev(ISEED)*2*PI)
c         CRAN(II,1)=FCCS*EXP(AI*RAN2(ISEED)*2*PI)
       ...
       dlr_r=2*pi/(2*nwvnor*dlwh)
       r0_r=0e0
       rm_r=r0_r+(2*nwvnor-1)*dlr_r
       write(6,*) 'rm_r,dlr_r=',rm_r,dlr_r
       do ii=1,nwvnor
        write(51,*) real(cran(ii,1)),rimag(cran(ii,1))
       end do

c >>> FFP to spatial domain
       smk=0e0
       do ii=1,2*nwvnor
        wn=(ii-1)*dlwh
        cff(ii,1)=cran(ii,1)*exp(-ai*r0_r*wn)*sqrt(wn)
        if (ii.gt.1) smk=smk+dlwh*wn*cran(ii,1)
       end do
       call cvconj(cff(2,1),2,cff(4*nwvnor,1),-2,2*nwvnor-1)
       cff(2*nwvno+1,1) = cff(2*nwvno,1)
       call cfft(cff,4*nwvnor,1)
c >>> taper upper 20%
       nwt=2*nwvnor*0.2
       nw1=2*nwvnor-nwt
       ...
       acm=0e0
       do i_r=2,2*nwvnor
        r_r=r0_r+(i_r-1)*dlr_r
        cran(i_r,2)=cff(i_r,1)*dlwh/sqrt(2*pi*r_r)
        acm=acm+cran(i_r,2)*r_r
       end do
       acm=acm/(2*nwvnor-1)
       do i_r=2,2*nwvnor
        r_r=r0_r+(i_r-1)*dlr_r
        cran(i_r,2)=cran(i_r,2)-acm/r_r
       end do
       cran(1,2)=smk

       do i_r=1,nwvnor
        write(52,*) real(cran(i_r,2)),rimag(cran(i_r,2))
       end do

c >>> FFT back to wavenumber domain
       call cvmov(cran(1,2),2,cff(1,1),2,2*nwvnor)
       call cvmov(cran(2,2),2,cff(4*nwvnor,1),-2,2*nwvnor-1)
       cff(2*nwvnor+1,1)=cff(2*nwvnor,1)
       call cfft(cff(1,1),4*nwvnor,-1)
       do i_w=1,nwvnor
        cran(i_w,1)=(-ai*2/pi)*cff(i_w,1)*dlr_r
       end do
       cran(1,1)=sqrt(real(cran(2,1)*conjg(cran(2,1))))

C *** PREPARE BESSEL FUNCTIONS FOR Le Page transform 

       brk_0=max(0e0,mbf_0-10.*pi)
       wnlp=(10*pi+mbf_0)/rm_r
       nwlp=wnlp/dlwh+1
       write(6,*) 'PREPBF'
       CALL PREPBF(rm_r,wnlp)
...
c >>> Lepage transform  back to wavenumber domain
       ...
       do i_w=1,nwlp
        ...
        do i_r=1,2*nwvnor
         r_r=r0_r+(i_r-1)*dlr_r
         argh=0.5*r_r*wn
         bes_J_0 =  RINTPL(BF(1,1),0E0,ONODRK,NRKMAX,argh)
         bes_Y_0 =  RINTPL(BF(1,2),0E0,ONODRK,NRKMAX,argh)
c         write(99,*) argh,bes_J_0,bes_Y_0
c         write(6,*) i_r,cran(i_r,2),r_r,bes_J_0,bes_Y_0,dlr_r
         if (i_r.ne.1.and.i_r.ne.nwvnor) then
          cran(i_w,3)=cran(i_w,3)+cran(i_r,2)*r_r
     &         *cmplx(bes_J_0,-bes_Y_0)*cmplx(bes_J_0,-bes_Y_0)*dlr_r
         else
          cran(i_w,3)=cran(i_w,3)+0.5*cran(i_r,2)*r_r
     &         *cmplx(bes_J_0,-bes_Y_0)*cmplx(bes_J_0,-bes_Y_0)*dlr_r
         end if
        end do
        if (nwlp.lt.100) then
         write(6,*) 'ik = ',i_w,' of ',nwlp
        else if (mod(i_w,nwlp/10).eq.0) then
         write(6,*) 'ik = ',i_w,' of ',nwlp
        end if
       end do
       smk=0e0
       do ii=2,5
        smk=smk+cran(ii,3)*conjg(cran(ii,3))
       end do
       cran(1,3)=sqrt(smk/4.0)

       do i_w=1,nwvnor
        fck=1e0/max(dlwh,(i_w-1)*dlwh)       
        awn=(i_w-1)*dlwh
        if (awn.lt.wnlp) then
         cdummy = (awn*cran(i_w,1)*fck +(wnlp-awn)*cran(i_w,3))/wnlp
        else
         cdummy = cran(i_w,1)*fck
        end if
        write(53,*) real(cdummy),rimag(cdummy)
       end do

      end if

c >>> convert to magnitude and phase
c      do jj=1,3,2
c       call cv2mp(cran(1,jj),2,cran(1,jj),2,2*nwvnor)
c      end do
...
c >>> Must be fixed for volume scattering formulation
      IF (AUSAMP.and.(.not.volume)) THEN
C ***  AUTOMATIC SAMPLING
       WRITE(6,*)
...
C ***   WAVENUMBER INTEGRATION
     ...
     &              NWVNO,ICW1,ICW2,ibody)
       END IF
c >>> scattering kernels not symmetric
       if (cylgeo) then
c >>> Cylindrical geometry only positive wavenumbers
        icut1=1
        icut2=nwvno
...
c >>> Plane geometry: scattering kernels not symmetric
       ...
       IF (ICNTIN.GT.0) THEN
        if (INTTYP.eq.2) then
         OFFDB=20.0*V(LAYS((LS-1)/2+1),2)*(1E0/CMIN-1E0/CMAX)/NWVNO
        else
         OFFDB=60.0*V(LAYS((LS-1)/2+1),2)*(1E0/CMIN-1E0/CMAX)/NWVNO
        end if
        WRITE(6,*) 'DEFAULT CONTOUR OFFSET APPLIED,',OFFDB,
     &             ' dB/wavelength'
      ...
      if (volume) then
       ...
       CALL CALSIC(CRAN(1,1),cran(1,3),wnlp,dlwh,2*nwvnor)
      else
       CALL CALSIP(CRAN(1,1),dlwh,2*nwvnor)
      end if
