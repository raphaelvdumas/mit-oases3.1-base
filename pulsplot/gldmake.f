     ...
     &                           CHAR(48+MOD(IPLCNT,10))//'.asc'
      ...
      read(5,'(a)') gldname
      gldname=gldname(1:lenstr(gldname))//'.asc'
...
c >>> Plain Binary format
      ...
      INCLUDE 'comgldwr.h'
        IUNGLD=18
        GLDNAME=FILEN(1:LFILNM)//CHAR(48+IPLCNT/10)//
     &                           CHAR(48+MOD(IPLCNT,10))//'.bin'
      write(6,*) 'BINARY Trace file name?'
      read(5,'(a)') gldname
      gldname=gldname(1:lenstr(gldname))//'.bin'
      OPEN(UNIT=IUNGLD,FILE=GLDNAME,
     &     FORM='UNFORMATTED',
     &     STATUS='UNKNOWN',ERR=999)
      RETURN
      ENTRY BINCLS()
      CLOSE(UNIT=IUNGLD,STATUS='KEEP')
      write(6,'(a,a)') 'Wrote BINARY trace file: ',gldname
      iplcnt=iplcnt+1
      RETURN
 999  STOP '>>> ERROR IN BINOPN CREATING BINARY FILE <<<'
      END
      SUBROUTINE BINMAKE(T0,FSMP,NPL,NR,NC)
      INCLUDE 'comgldwr.h'
...
c
      ...
      SUBROUTINE BINWRI(DATA,nc)
      INCLUDE 'comgldwr.h'
      DIMENSION DATA(1)

      write(iungld) (data(j), j=1,nc)  
