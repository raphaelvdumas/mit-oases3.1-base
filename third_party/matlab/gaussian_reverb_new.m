%[reverb]=gaussian_reverb_new(wo,d_w,phi,Z,Zs,Zr,phi_ib,phi_rb,Cp,Cg,D,T,ell,flag)
%
...
sig=d_w^(-2)-i*(D*ones(1,M)+ones(M,1)*D.')*t.*Snm.^(-1)/4;
...
two=(t*snm.*(1+s2./s1)./s1_s2.*(1+((1+s2./s1)*ell./s1_s2.*snmp.*snm+(1+s2./s1)./(1+s1./s2).*snm./snmp).^(-1))+i*(knm-knmp.*(1-(1+(ell*snmp.^2.*rat2).^(-1)).^(-1)))).^2./(2*snm.^2.*(1+s2./s1)./s1_s2+2/ell*(1+(ell*snmp.^2.*rat2).^(-1)).^(-1));
