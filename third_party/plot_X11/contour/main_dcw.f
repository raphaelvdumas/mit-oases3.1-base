C   MAIN.FOR
     ...
     & NPXYZ=25500,NLEV1=51, NDUMMY= 20)
      parameter (nbot_max = 5000, nbm= nbot_max+20)
      LOGICAL IB, BATCH
      INTEGER TTYPE,GT$TTY
      REAL LASPX, LASPY
      CHARACTER*3 NEW, ULC
     ...
     & ADD, OPENED, LASTP
      ...
      CHARACTER*13 WDWMSG
      CHARACTER*80 OPTION,INPFILE

      REAL*8 X, Y, S

      COMMON /BOTT/ XF(nbm), YF(nbm), NPBOTT, ISHADE, NPSH
      COMMON /MULBOTT/ LINES
      COMMON /DEV/ DEVICE
     ...
     & (WRK(881),UF(1)), (WRK(1321),VF(1))
      ...
      DATA WDW,WDWMSG/'DCW','DEC WINDOW   '/
...
C
  ...
  500 FORMAT(1H0,'Do you want a hardcopy?',
     &       /1H ,'Printronix (black & white) : PRX',
     &       /1H ,'Tektronix 4698 Phaser3 (A3): TEK',
     &       /1H ,'Tektronix 4693 (A4)        : T93',
     &       /1H ,'Laser Printer              : LAS',
     &       /1H ,'Viewgraph 4692             : VUG',
     &       /1H ,'Tek Phaser PX              : PHA',
     &       /1H ,'Encaps Postscript file     : EPS',
     &       /1H ,'Postscript B/W (A4)        : PS4',
     &       /1H ,'Postscript B/W (A3)        : PS3',
     &       /1H ,'None                       : N' )
  600 FORMAT(1X,//,' Z GRID IS NOW UNDERSAMPLED BECAUSE OF '
     ...
     & //)
  700 FORMAT(1X,//,' *** WARNING: CONTUR COMMAND IGNORED *** ',/,
     & '  CONTOUR FILE NOT AVAILABLE ',//)
C
      IB= BATCH()
C
      IFIRST=1
      OPEN(UNIT=55,STATUS='OLD',SHARED,ERR=9000)
 1000 CONTINUE
1     READ(55,200,ERR=9100) OPTION,(DUMMY(J),J=2,NDUMMY)
...
C
      ...
      ELSE IF( (DUMMY(J) .EQ. 'TEK') .OR. (DUMMY(J) .EQ. 'tek') )  THEN
        DEVICE= 'TEK'
      ELSE IF( (DUMMY(J) .EQ. 'T93') .OR. (DUMMY(J) .EQ. 't93') )  THEN
        DEVICE= 'T93'
        PACKGE= 'UNI'
      ...
      ELSE IF( (DUMMY(J) .EQ. 'EPS') .OR. (DUMMY(J) .EQ. 'eps') )  THEN
        DEVICE= 'EPS'
      ELSE IF( (DUMMY(J) .EQ. 'PS4') .OR. (DUMMY(J) .EQ. 'ps4') )  THEN
        DEVICE= 'PS4'
      ELSE IF( (DUMMY(J) .EQ. 'PS3') .OR. (DUMMY(J).EQ.'ps3') )    THEN
        DEVICE= 'PS3'
...
C
      IF( DEVICE .EQ. 'LAS' .OR. DUMMY(J) .EQ. 'las' )    BWCOL='B/W'

      IF(VUG.EQ.'VUG')   THEN
       PACKGE='UNI'
       DEVICE='TEK'
       BWCOL='COL'
      ELSE IF( BWCOL .EQ. 'COL' )   THEN
       PACKGE='UNI'
       IF(( DEVICE. EQ. 'PRX' ) .OR.
     &    ( DEVICE .EQ. 'CAL'))
     & DEVICE='TEK'
      ELSE IF( DEVICE .EQ. 'CAL' )   THEN
       PACKGE='MIN'
       BWCOL='B/W'
      END IF
C
C
      IF( PACKGE .NE. 'UNI' )   WRITE(6,420) PACKGE,BWCOL,DEVICE
C
C
CC ********************************************************
C   LOOP FOR CONDR, TDPEN, AND CONUS OPTION
C
       ...
       ELSE
        IF((NX.NE.NPX).OR.(NY.NE.NPY))   THEN
         IF((NX.LE.NPX).AND.(NY.LE.NPY))   THEN
          LU=22
          OPEN(UNIT=LU,FILE='DUMMY1',STATUS='SCRATCH',
     &         FORM='UNFORMATTED')
...
C
     ...
     & .OR.OPTION(1:5).EQ.'IFD  ')   THEN
       ...
       IF(INPFILE(J+1:J+3) .EQ. 'DAT')   THEN
        CALL IPEOLD(Z,NZX,NZY,FREQ,SD,FORM)
        NPX=NX
        NPY=NY
       ELSE 
...
C
       IF (.NOT.IB) THEN
        ...
        X1PL=XSAVE
        Y1PL=YSAVE
        IF(NEW.EQ.'B/W' .OR. NEW.EQ.'b/w')   THEN
         PACKGE='UNI'
         BWCOL='B/W'
         CALL BACKBOTT
        ELSE IF(NEW.EQ.'LAS' .OR. NEW.EQ.'las')   THEN
         ...
         LINEP='LIN'
         CALL BACKBOTT
        ELSE IF(NEW.EQ.'COL'.OR.NEW.EQ.'col')   THEN
         PACKGE='UNI'
         BWCOL='COL'
         CALL BACKBOTT
        END IF
     ...
     & YPIXEL,VUG,DEL,WRK,NPXYZ,FAX,ROT,REVERS,NCS,WDW)
       ...
       IF (DEVICE.EQ.'T41' .OR. DEVICE.EQ.'VTT') THEN
        ...
        ELSE IF (NEW.EQ.'TEK' .OR. NEW.EQ.'tek')   THEN
         ...
         CALL BACKBOTT
         GO TO 555
        ELSE IF (NEW.EQ.'T93' .OR. NEW.EQ.'t93')   THEN
         PACKGE='UNI'
         DEVICE='T93'
         CALL BACKBOTT
         GO TO 555
...
C         BWCOL ='B/W'
         ...
         CALL BACKBOTT
         GO TO 555
        ELSE IF (NEW.EQ.'PS4' .OR. NEW.EQ.'ps4')   THEN
         PACKGE='UNI'
C         BWCOL ='B/W'
         DEVICE='PS4'
         CALL BACKBOTT
         GO TO 555
        ELSE IF (NEW.EQ.'PS3' .OR. NEW.EQ.'ps3')   THEN
         PACKGE='UNI'
C         BWCOL ='B/W'
         DEVICE='PS3'
         CALL BACKBOTT
