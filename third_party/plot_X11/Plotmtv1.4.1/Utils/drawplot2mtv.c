/*
 * drawplot2mtv.c - program to convert from drawplot-format data to plotmtv
 */
...
char **argv;
{
   ...
   for (i=1; i<argc; i++) {
      ...
      if (argv[i][0] != '-' && argv[i][0] != '=') { /* input file */
         ...
         (void) fprintf(stdout,
                "\nReading drawplot data from \"%s\"...\n",datafile);
         read_drawplotdata(datafile);
...
/* 
 * Read the datafile in drawplot format 
 */
static void read_drawplotdata(filename)
char *filename;
{   
   CNdatasetptr dptr;
   ...
   dptr = CNread_plot2D(filename,(FILE *)NULL,CN_FILE,
                         CN_FALSE, 1, 0);
