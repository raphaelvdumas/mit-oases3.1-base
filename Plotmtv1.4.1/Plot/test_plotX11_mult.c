/*
 * test_plotX11_mult.c - X11 routines for initializing an X11/Xlib application
 *                       These routines are used for creating a window
...
#define NWINDOWS        16
#define WINOFFSET        2
static int         plotwin_width[NWINDOWS];
static int         plotwin_height[NWINDOWS];
static int         plotwin_xoffset[NWINDOWS];
static int         plotwin_yoffset[NWINDOWS];
static int         nplotwins;
...
static Window      vwwindows[NWINDOWS];        /* drawing windows   */
static Window      buttons[NBUTTONS];          /* button windows    */
static GC          gc0, gc1, gcxor;            /* graphics context  */
static GC          bgc, bgcr;                  /* graphics context  */
static GC          gcbd, gcbdxor;              /* gc for border     */
static Pixmap      vwpixmaps[NWINDOWS];        /* pixmaps for plots */
...
static int         create_mult_window();
static void        calc_button_dim();
static void        calc_plotwin_dim();
...
/*
 * Special flags for controlling order of plots in page
 */
static int    force_vertical;
 ...
 */
int PXcreateXMultWindow(pshead, pstail, progname, display_name,
                        plot_foreground, plot_background, nopixmap, 
                        x_psfile, x_printCmd, x_printer,
                        x_scale, x_drawframe, 
                        x_landscape, x_printdate, x_colorps,
...
int    force_vert;             /* Force vertical plot   */
int    debug;                  /* Debug flag            */
{
   ...
   nplotwins = CNcount_pslists(pshead, pstail);
   if (nplotwins <= 0) {
      (void) fprintf(stderr, "Error! Nothing to plot!\n");
      status = 0;
      return(status);
   } else if (nplotwins > NWINDOWS) {
      (void) fprintf(stderr, "Warning! Can only place a maximum of ");
      (void) fprintf(stderr, "%d plots in a single window!\n",NWINDOWS);
      nplotwins = NWINDOWS;
   }
   ...
   /* Copy the mult-window variables */
   force_vertical = force_vert;

   /*
    * Try to open the window first; proceed with plot if successful
    */
   if ((status = create_mult_window(progname,display_name,
 ...
 * Create an X11 window
 */
static int create_mult_window(progname,display_name,
                              plot_foreground,plot_background,
                              debug)
char *progname;              /* Program name     */
char *display_name;          /* Display name     */
...
int  debug;                  /* Debug flag       */
{
   ...
   /*
    * Different default geometry for landscape mode 
    */
   ...
   if (!landscape) {
      ...
      /* Default geometry */
      (void) sprintf(default_geometry,"=%dx%d+%d+%d",Y_DIM,X_DIM,X_ORG,Y_ORG);
   }
   ...
   /* Calculate the plot-window dimensions */
   yoffset = button_yoffset[NBUTTONS-1]+button_height[NBUTTONS-1] + OFFSET;
   calc_plotwin_dim(OFFSET, yoffset, width-2*OFFSET, height-(yoffset+OFFSET), 
                    plotwin_width, plotwin_height,
                    plotwin_xoffset, plotwin_yoffset);

   /* Create the drawing windows */
   for (i=0; i<nplotwins; i++) {
      vwwindows[i] = XCreateSimpleWindow(mydisplay, rtwindow,
                    plotwin_xoffset[i], plotwin_yoffset[i],
                    plotwin_width[i],   plotwin_height[i], 1,
                    foreground_pixel, background_pixel);
   }
   ...
   for (i=0; i<nplotwins; i++) {
      XSelectInput(mydisplay,vwwindows[i],
                   ExposureMask |
                   ButtonPressMask | ButtonMotionMask | ButtonReleaseMask | 
                   KeyPressMask );
   }
   ...
   if (use_pixmap) {
      ...
      for (i=0; i<nplotwins; i++) {
         vwpixmaps[i] = XCreatePixmap(mydisplay, RootWindow(mydisplay,screen), 
                                plotwin_width[i],   
                                plotwin_height[i],
   ...
   gc0 = XCreateGC(mydisplay, rtwindow, 0L, &gcvalues);
   gc1 = XCreateGC(mydisplay, rtwindow, 0L, &gcvalues);
   ...
   gcxor = XCreateGC(mydisplay, rtwindow, GCFunction, &gcvalues);
   XSetForeground(mydisplay, gcxor, foreground_pixel ^ background_pixel);

   /* Create GC for indicating active view-window */
   gcvalues.function = GXclear;
   gcbd = XCreateGC(mydisplay, rtwindow, 0L, &gcvalues);
   gcvalues.function = GXxor;
   gcbdxor = XCreateGC(mydisplay, rtwindow, GCFunction, &gcvalues);
   XSetForeground(mydisplay, gcbdxor, foreground_pixel ^ background_pixel);
   XSetLineAttributes(mydisplay,gcbd   ,3,LineSolid,CapButt,JoinBevel);
   XSetLineAttributes(mydisplay,gcbdxor,3,LineSolid,CapButt,JoinBevel);
...
/* 
 * Calculate the position and width of plot-windows
 */
static void calc_plotwin_dim(rtwin_xoffset, rtwin_yoffset, 
                             rtwin_width, rtwin_height,
                             plotwin_width, plotwin_height,
                             plotwin_xoffset, plotwin_yoffset)
int rtwin_xoffset;
int rtwin_yoffset;
...
int plotwin_yoffset[];
{
   int single_plot_width, single_plot_height, i, j, nx, ny;
   int nplots;

   /* 
    * The size of the windows is dictated by the width/height ratio of the
    * main window, as well as by the number of plots
    */
   nplots = nplotwins;
    ...
    */
   if (force_vertical) {
      /* Force the plots to be laid out vertically (only 1 column) */
      if (nplots <= 3) {
         nx     = 1;
         ny     = nplots;
      } else if (nplots <= 6) {
   ...
   single_plot_width  = (int) ( (rtwin_width ) / (double)(nx));
   single_plot_height = (int) ( (rtwin_height) / (double)(ny));

   /* Now go through and initialize each plot window */
   if (force_vertical) {
      /*
      ...
      for (i=0; i<nx; i++) {
         for (j=0; j<ny; j++) {
            plotwin_width  [j+i*ny] = single_plot_width  - 1;
            plotwin_height [j+i*ny] = single_plot_height - 1;
            plotwin_xoffset[j+i*ny] = rtwin_xoffset + i*single_plot_width;
            plotwin_yoffset[j+i*ny] = rtwin_yoffset + j*single_plot_height;
         }
      }
      ...
      for (i=0; i<nx; i++) {
         for (j=0; j<ny; j++) {
            plotwin_width  [i+j*nx] = single_plot_width  - 1;
            plotwin_height [i+j*nx] = single_plot_height - 1;
            plotwin_xoffset[i+j*nx] = rtwin_xoffset + i*single_plot_width;
            plotwin_yoffset[i+j*nx] = rtwin_yoffset + j*single_plot_height;
         }
      }
   ...
   for (i=0; i<NWINDOWS; i++) {
      (void) printf("Plot-window %d : x=%3d y=%3d width=%3d height=%3d\n",i,
                    plotwin_xoffset[i], plotwin_yoffset[i], 
                    plotwin_width[i], plotwin_height[i]);
   }
    */
}
...
int          debug;
{
   ...
   /* Initialize the active plotset and window */
   pptr = pshead->Pptr;
   activeWinID = 0;
   ...
   while(1) {
      ...
      case Expose:
         ...
         /* If the exposed window is one of the plot windows... */
         psptr = pshead;
         for (i=0; i<nplotwins; i++) {
            if (event.xexpose.window == vwwindows[i]) {
               show_view(psptr->Pptr, &(redraw[i]), 
                         vwwindows[i], vwpixmaps[i],
                         plotwin_width[i], plotwin_height[i], debug);
            } 
            psptr = psptr->next;
         }
         ...
         /* Draw a rectangle about the active view-window */
         XDrawRectangle(mydisplay, vwwindows[activeWinID], gcbd, 0, 0,
                        plotwin_width[activeWinID],
                        plotwin_height[activeWinID]);
      ...
      case ConfigureNotify:
         ...
         calc_plotwin_dim(OFFSET, yoffset, VwWidth, VwHeight,
                          plotwin_width, plotwin_height,
                          plotwin_xoffset, plotwin_yoffset);
         for (i=0; i<nplotwins; i++)
            XMoveResizeWindow(mydisplay,vwwindows[i],
                              plotwin_xoffset[i], plotwin_yoffset[i],
                              plotwin_width[i], plotwin_height[i]);
         ...
         if (use_pixmap) {
            for (i=0; i<nplotwins; i++) {
               if (vwpixmaps[i]) XFreePixmap(mydisplay, vwpixmaps[i]);
               vwpixmaps[i] = XCreatePixmap(mydisplay,
                                         RootWindow(mydisplay, screen),
                                         plotwin_width[i], plotwin_height[i],
                                         DisplayPlanes(mydisplay,screen));
               XSetForeground(mydisplay, gc0, background_pixel);
               XFillRectangle(mydisplay, vwpixmaps[i], gc0, 0, 0, 
                              plotwin_width[i], plotwin_height[i]);
               XSetForeground(mydisplay, gc0, foreground_pixel);
            }
         }
      ...
      case KeyPress:
         ...
         /*
          * If the key was pressed in a sub-window, then make
          * that sub-window the active window 
          */
         /* Check for the window in which this happens */
         psptr = pshead;
         for (i=0; i<nplotwins; i++) {
            if (event.xbutton.window == vwwindows[i]) {
               /* Remove the rectangle about the current view-window */
               XDrawRectangle(mydisplay, vwwindows[activeWinID], gcbdxor, 0, 0, 
                              plotwin_width[activeWinID],
                              plotwin_height[activeWinID]);
         ...
         if ((keysym == XK_q) || (keysym == XK_Q)) {
            ...
            rotate_picture(pptr,&eyepos,EVL,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID], 
                           &(redraw[activeWinID]));
         else if ((keysym == XK_k) || (keysym == XK_K))
            rotate_picture(pptr,&eyepos,EVK,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
         else if ((keysym == XK_j) || (keysym == XK_J))
            rotate_picture(pptr,&eyepos,EVJ,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
         else if ((keysym == XK_h) || (keysym == XK_H))
            rotate_picture(pptr,&eyepos,EVH,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
         else if ((keysym == XK_o) || (keysym == XK_O))
            rotate_picture(pptr,&eyepos,EVO,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
         else if ((keysym == XK_f) || (keysym == XK_F))
            rotate_picture(pptr,&eyepos,EVF,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
         else if ((keysym == XK_d) || (keysym == XK_D))
            rotate_picture(pptr,&eyepos,EVD,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
         else if ((keysym == XK_s) || (keysym == XK_S))
            rotate_picture(pptr,&eyepos,EVS,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
         else if ((keysym == XK_a) || (keysym == XK_A))
            rotate_picture(pptr,&eyepos,EVA,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
         else if ((keysym == XK_x) || (keysym == XK_X))
            rotate_picture(pptr,&eyepos,EVX,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
         else if ((keysym == XK_y) || (keysym == XK_Y))
            rotate_picture(pptr,&eyepos,EVY,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
         else if ((keysym == XK_z) || (keysym == XK_Z))
            rotate_picture(pptr,&eyepos,EVZ,
                           vwwindows[activeWinID],
                           plotwin_width[activeWinID],
                           plotwin_height[activeWinID],
                           &(redraw[activeWinID]));
      ...
      case ButtonPress:
         ...
         if (check_buttons(&event, pptr, pshead, pstail, &eyepos, &close_down, 
                           vwwindows[activeWinID],
                           VwWidth, VwHeight, &(redraw[activeWinID]), debug)) {
            if (close_down) {
         ...
         paint_button(buttons[QUIT_BUTTON],NORMAL,QUIT_BUTTON,pptr);
         ...
         PXDrawRectangle(mydisplay, vwwindows[activeWinID], gcxor, 
      ...
      case ButtonRelease:
         ...
         if (event.xbutton.button == button) {
            ...
            last_y = event.xbutton.y;
            /* Redraw */
            PXDrawRectangle(mydisplay, vwwindows[activeWinID], gc, 
                           start_x, start_y, last_x-start_x, last_y-start_y);
            zoom_in(pptr, button, start_x, start_y, last_x, last_y, 
                    vwwindows[activeWinID], 
                    VwWidth, VwHeight, &(redraw[activeWinID]));
         }
         break;
 ...
 * to redraw
 */
static int check_buttons(event, pptr, pshead, pstail, eyepos, close_down, 
                         vwwindow, VwWidth, VwHeight, redraw, debug)
XEvent       *event;
CNplotsetptr pptr;
CNpslistptr  pshead, pstail;
CNcoord      *eyepos;
 ...
 * Rotate the view 
 */
static void rotate_picture(pptr,eyepos,mode,
                           vwwindow, VwWidth, VwHeight, redraw)
CNplotsetptr pptr;
CNcoord      *eyepos;
int          mode;
Window       vwwindow;
...
static void show_view(pptr, redraw, vwwindow, vwpixmap,
                      VwWidth, VwHeight, debug)
CNplotsetptr pptr;
int          *redraw;
Window       vwwindow;
Pixmap       vwpixmap;
...
int          debug;
{
   ...
   if (use_pixmap) {
      ...
      if (*redraw) {
         ...
         /* Set to watch cursor */
         xs_watch_cursor(mydisplay, rtwindow);

         /* Reset the pixmap */
         XSetForeground(mydisplay, gc0, background_pixel);
         XFillRectangle(mydisplay, vwpixmap, gc0, 0, 0, VwWidth, VwHeight);
         XSetForeground(mydisplay, gc0, foreground_pixel);

         /* Draw the plot */
         (void) PXdrawplotX(pptr,
                            mydisplay, vwwindow, vwpixmap,
                            gc0, gc1, font0, font1, 
                            VwWidth, VwHeight,
         ...
         *redraw = CN_FALSE;
         ...
         xs_normal_cursor(mydisplay, rtwindow);
          ...
          */
         XCopyArea(mydisplay, vwpixmap, vwwindow, gc0, 0, 0,  VwWidth,
                   VwHeight, 0, 0);
      }
   } else { /* if (!use_pixmap) */
      /*
