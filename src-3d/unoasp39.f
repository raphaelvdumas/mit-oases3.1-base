...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C     
C     3-d - TRANSVERSE ISOTROPY - PULSE VERSION          
...
C *** MODIFY FREQUENCY SAMPLING FOR AUTOMATIC SAMPLING
      IF (AUSAMP) THEN
       ...
       RM=AMAX1(ABS(R0),ABS(R0+(NPLOTS-1)*RSPACE))*1E3
       TREQ=RM/CREF
 989   NX=MAX(NX,nint(TREQ/DT))
       IPOW=0
 990   IPOW=IPOW+1
       II=2**IPOW
...
C *** PREPARE BESSEL FUNCTIONS FOR FULL INTEGRATION
      IF (INTTYP.EQ.2) THEN
c       CALL PREPBF(RRMAX,WKMAX*1.2)
c       ICDR=1
c >>> v2.2 has tapered full Bessel below kr=50*pi
       mbf_0=0
       brk_0=max(0e0,mbf_0-10.0*pi)
       bfrmax=(10*pi+mbf_0)/wkmax
       CALL PREPBF(bfrmax,WKMAX)
c       bfrmax=50*pi/wkmax
c       CALL PREPBF(bfrmax,WKMAX)
       ICDR=0
