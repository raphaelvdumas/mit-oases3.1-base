      SUBROUTINE STATIST(Z,NPXRD,NX,NPYRD,Z1,Z2,Z3,SIGMA,RMAX,
     & COEFF)
...
C
      ...
      AT100=ABS(T) * 100.0
      L= MIN(399.0,AT100) + 1
      AREA= 0.5 + TABLE(L)*SIGN(1.0,T)
C
      BUFF(K)=BUFF(K) + COEFF(J)*AREA
 2000 CONTINUE
 3000 CONTINUE
      DO 4000   L=1,NX
      BUFF(L)=BUFF(L)*1.0E-3*RMAX/((NPXRD-1)*DNORM)
 4000 CONTINUE
      WRITE(LU)(BUFF(L),L=1,NX)
C
 5000 CONTINUE
      REWIND LU
      NPXRD=NX
      CALL ZIN(Z,NX,NPYRD,LU)
      CLOSE(LU)
      RETURN
      END
