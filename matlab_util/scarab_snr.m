%[amb,snr]=scarab_snr(filename,chan,win,stat_flag,jump)
%
% Kevin D. LePage
% 17/4/98
% SACLANTCEN
%
% function to compute the scarab reverb products
%
% Inputs:
%
...
%fprintf('Please click on the beginning of the ambient noise:')

%[vecx1,vecy]=ginput(1);

%fprintf('\n')

%fprintf('Please click on the beginning of the signal:')

%[vecx2,vecy]=ginput(1);

%fprintf('\n')

%fprintf('Please click on the end of the signal:')

%[vecx3,vecy]=ginput(1);

%fprintf('\n')

%vecx=[vecx1;vecx2;vecx3];
...
% find the relevant octave bands

[low,high,bands]=third_octave(h_pass,fs/3);
...
spec=ts_fft(ind_low:ind_high,:).*((ts_fft_source(ind_low:ind_high))'*ones(1,jump));
...
eval(['save ' fname '_snr snr amb times chan bands win mean_win source fs h_pass gain kolsmir nblock'])
