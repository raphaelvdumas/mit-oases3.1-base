%[image_out]=image_echo(ts,replica,fs,xs,co,x,z,thresh)
%
...
function[image_out]=image_echo(ts,replica,fs,xs,co,x,z,thresh)
...
ts_mf=abs(hilbert(ifft(fft(ts).*(conj(fft(replica))*ones(1,size(ts,2))))));
...
ts=fft(ts,nfft).*(replica*ones(1,size(ts,2)));
...
image_out=zeros(size(ts,2),length(r));
